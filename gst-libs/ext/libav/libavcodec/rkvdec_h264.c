...
static int rkvdec_h264_regs_gen_reg(AVCodecContext *avctx)
{
    ...
    hw_regs->swreg3_picpar.sw_y_hor_virstride = ALIGN(pic->f->width * (pp->bit_depth_luma_minus8 + 8) / 8, 256) / 16;
    hw_regs->swreg3_picpar.sw_uv_hor_virstride =  ALIGN(pic->f->width * (pp->bit_depth_chroma_minus8 + 8) / 8, 256) / 16;
    hw_regs->swreg8_y_virstride.sw_y_virstride = ALIGN(ALIGN(pic->f->width * (pp->bit_depth_luma_minus8 + 8) / 8, 256) * ALIGN(pic->f->height, 16), 16) / 16;
    hw_regs->swreg9_yuv_virstride.sw_yuv_virstride = ALIGN(ALIGN(pic->f->width * (pp->bit_depth_chroma_minus8 + 8) / 8, 256) * ALIGN(pic->f->height, 16) * 3 / 2, 16) / 16;
